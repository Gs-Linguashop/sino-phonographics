...
# in this script 
# the main algorithm is performed on ttf file 
# it is more time consuming if the font is compiled from ufo 
# but more direct if the font is provided directly in ttf 
...
# the following part compiles the ufo's into ttf 
# if one needs to supplement extra glyphs to the original source file, both files has to be in ufo format 
# it shall be commented out if the source file is ttf/otf 
# '''
...
font = compileTTF(ufo)
# ''' 
# the part above compiles the ufo's into ttf 

# the following line reads ttf/otf source file directly 
# this line shall be used only when the source file is ttf/otf 
# however, one cannot supply supplementary glyphs through this method 
...
for cmap,ori_cmap in zip(font['cmap'].tables,copy.deepcopy(font['cmap'].tables)):
    ...
    for char in forest.dict:
        ...
        if is_BMP and ord(char) > 65535: continue
        map_to_char_and_glyph = forest.dict[char].find_substitution_glyph(forest, displayed_chars.dict, not_displayed_chars.dict, subs, best_cmap.get, missing_glyphs, False) # is_BMP)
        if map_to_char_and_glyph is None: continue
        map_to_char, cmap.cmap[ord(char)] = map_to_char_and_glyph
